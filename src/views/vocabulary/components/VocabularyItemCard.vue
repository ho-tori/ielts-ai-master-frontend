    ...
    <div class="flex items-start justify-between gap-3">
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-2">
          <h4 class="font-semibold text-text-primary text-lg">{{ item.word }}</h4>
          <span class="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary">{{ item.semanticCluster }}</span>
        </div>
        <div v-if="item.questionReplacement" class="text-sm mb-2">
          <span class="text-text-secondary">题目替换词：</span>
          <span class="text-primary font-medium">{{ item.questionReplacement }}</span>
        </div>
