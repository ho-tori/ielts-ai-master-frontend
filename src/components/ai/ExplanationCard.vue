<template>
  <BaseCard>
    <template #header>
      <h3 class="font-semibold text-text-primary">AI 详细解析</h3>
    </template>
    ...
    <div v-else-if="result" class="space-y-6">
      <div>
        <h4 class="font-semibold text-text-primary mb-2 text-sm">解释</h4>
        <p class="text-text-secondary leading-relaxed">{{ result.explanation }}</p>
      </div>
      ...
      <div v-if="result.relatedQuestions" class="pt-2 border-t border-border/70">
        <h4 class="font-semibold text-text-primary mb-2 text-sm">相关提问</h4>
    ...
    <div v-else class="text-center text-text-secondary/70 py-8">暂无数据</div>
  </BaseCard>
</template>
